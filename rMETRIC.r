...
aoi<-createAoi(topleft = c(246419.3,6081628 ),
               bottomright =c( 253965, 6074972) , EPSG = 32719)
...
WeatherStation <- read.WSdata(WSdata = csvfile,
                              lat=-35.38255, long= -71.713367, elev=104, height= 5.6,
                              columns = c("date","time","radiation","wind","RH","temp"),
                              time.format="%H:%M:%S", date.format = "%d/%m/%Y", MTL = MTLfile)
...
#plot(solar.angles.r)
Rs.inc <- incSWradiation(surface.model = surface.model, solar.angles = solar.angles.r, 
                         WeatherStation = WeatherStation)

image.TOAr <- calcTOAr(image.DN = image.DN, sat = "L7", 
                       MTL = MTLfile, incidence.rel = solar.angles.r$incidence.rel)
...
image.SR <- calcSR(image.TOAr=image.TOAr, sat = "L7", 
                   ...
                   WeatherStation=WeatherStation)
#plot(image.SR)
albedo <- albedo(image.SR = image.SR,  coeff="Liang", sat="L7")
plot(albedo)
...
hot.and.cold <- calcAnchors(image = image.TOAr, Ts = Ts, LAI = LAI, plots = F,
                            albedo = albedo, Z.om = Z.om, n = 2, 
                            anchors.method = "CITRA-MCBbc", deltaTemp = 5, 
                            WeatherStation = WeatherStation, verbose = FALSE)

H <- calcH(anchors = hot.and.cold, Ts = Ts, Z.om = Z.om, 
           WeatherStation = WeatherStation, ETp.coef = 0.5,
           Z.om.ws = 0.3, DEM = dem, Rn = Rn, G = G, verbose = FALSE)
...
plot(ET.24, xlab="Easting", ylab="Northing",main="24 Hour Evapotranspiration",
     col=rainbow(255))
summary(ET.24)

writeRaster(ET.24, filename="ET24h_06_Ene2010_50km2_KcCal.tif",datatype='FLT4S',
            overwrite=T)
